...
export const action: ActionFunction = async ({ request }) => {
    ...
    const externalApiUrl = `${process.env.API_URL}/hoprs/search`; // Replace with your external API URL
    const externalFormData = new FormData();
    //externalFormData.append('original_image_qt', new Blob([file]), 'demo.csv');
    //externalFormData.append('original_image_qt', fs.createReadStream(file));
    ...
    try {
      ...
      return jsonWithSuccess({ success: true, data: responseData }, "Image search successful!");
    } catch (error) {
        console.error(error)
...
export default function Search() {
    ...
    <div className="p-5 container m-auto text-center items-center justify-center flex flex-col flex-stretch min-h-screen pt-[120px] pb-[50px]">
      <div className="intro text-gray-500 lg:w-2/3 mx-auto mb-5 text-sm">
        <h1 className="font-bold text-3xl text-black">Image Search</h1>
        <p className="my-3">Use this tool to search HOPrS database for an image.</p>
      </div>
      <fetcher.Form method="POST" encType="multipart/form-data" className="flex flex-col w-full">
        ...
        <div className="buttons my-10">
            <LoadingButton loading={loading} width={300} className="bg-black">Search</LoadingButton>
        </div>
      </fetcher.Form>
